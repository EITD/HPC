...
int main(int argc, char *argv[]){
    ...
    if (size == 1) {
      ...
      if (rank == 0) {
        ...
        MPI_Recv(&left_ghost, 1, MPI_DOUBLE, (rank - 1 + size) % size, 0, MPI_COMM_WORLD, &status);

        MPI_Ssend(&f[1], 1, MPI_DOUBLE, (rank - 1 + size) % size, 0, MPI_COMM_WORLD);
        MPI_Recv(&right_ghost, 1, MPI_DOUBLE, (rank + 1) % size, 0, MPI_COMM_WORLD, &status);
      } else {
        MPI_Recv(&left_ghost, 1, MPI_DOUBLE, (rank - 1 + size) % size, 0, MPI_COMM_WORLD, &status);
        MPI_Ssend(&f[nxn_loc - 2], 1, MPI_DOUBLE, (rank + 1) % size, 0, MPI_COMM_WORLD);
